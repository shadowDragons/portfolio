...
- 既可以写技术交付与架构判断，也可以写范围控制、优先级和项目边界判断
- 标题 + 副标题要先把问题说清楚
- 正文中可融入一点技术实现细节或代码思路
...
- 如果用户没有指定写作视角，优先在“技术方案 / 交付 / 稳定性 / 维护”和“业务判断 / 产品规划 / 项目边界”两条线之间轮换
- 一次任务默认输出 1 条
- 只有用户明确要求批量时，才一次输出多条
...
- `视角 = 自动` 时，默认允许 AI 在“技术交付”和“业务判断 / 产品规划”两条线之间轮换
...
- 语言更像品牌、产品、流程或系统判断观察，不要写成工程实现教程
...
4. 如果用户没有指定写作视角，也不要长期只用单一视角，要在“技术交付”和“业务判断 / 产品规划”两条线之间轮换
5. 如果用户没有指定平台和语言，默认在知乎、掘金、即刻、小红书里任选 1 个平台
6. 输出的平台内容必须像原生内容，不像广告
7. 保留核心判断，但不要逐字复述站内原文
8. 一次只放 1 个最相关链接
9. 不夸张承诺，不伪造案例，不堆砌营销词，也不要把品牌写成低价外包
10. 即刻更像个人观察，知乎更像回答问题，掘金更像方法总结，小红书更像图文观点卡片，Medium 更像英文判断型长文
...
- 可以写技术交付判断，也可以写业务判断 / product planning / scope decision
...
默认让技术交付线和业务判断 / 产品规划线交替出现，不要连续多条都只用同一种视角。
...
1. 先跑知乎 1 篇：`website-first-or-system-first`
2. 再跑掘金 1 篇：`internal-system-refactor-or-rebuild`
3. 再跑即刻 1 条：`landing-page-vs-company-website`
...
4. 如果我没有指定写作视角，也不要长期只用单一视角，要在“技术交付”和“业务判断 / 产品规划”两条线之间轮换
...
9. 中文各平台调性：知乎像回答问题，掘金像方法总结，即刻像个人观察，小红书像观点型图文卡片，电鸭侧重外包接单踩坑与工作室高端展示。默认强调质量、服务和长期性价比，不打低价牌。抖音必须严格采用短视频口播神级公式（反常识标题+趋势压迫感开场+3到5个收益点+数据背书+软CTA）。
10. 英文：Medium/Dev.to 偏向技术与判断型长文；IndieHackers 侧重独立开发复盘；X 则是短推文。
11. 英文内容必须首选站内英文原文（`/en` 结尾路径的内容）去提炼，绝不能把中文稿逐句机械直译。
12. 不要像广告，不要夸张承诺，不要伪造案例，也不要写成低价外包叙事
13. 如果有历史记录，优先避开最近 7 天已经发过的文章 slug
