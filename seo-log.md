...
2. AI 每次补文时应该只改哪些文件
3. 选题池、随机规则和去重规则
4. slug、分类、relatedServices、priority、readingMinutes 规则
5. 给 AI 的主提示词和定时执行指令

这样做的原因：

- 当前博客是数据驱动，不是 Markdown 驱动
- 如果要让 AI 定时补文，必须让它理解 `src/lib/articles.ts` 的结构，而不是只会写正文
- 抽成独立 SOP 后，后面更容易接自动化任务
